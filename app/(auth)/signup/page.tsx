...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // After successful registration, redirect to next step
  ...
  const handleGoogleSignUp = async () => {
    setIsLoading(true)
    try {
      await signIn("google", { callbackUrl: "/signup/username" })
    } catch (error) {
  ...
  return (
    <AuthLayout>
      <div className="mb-8">
        ...
        <p className="mt-2 text-sm text-gray-600">Let's get you set up so you can create your personal account</p>
      </div>
      ...
      <form className="space-y-6" onSubmit={handleSubmit}>
        ...
        <div className="flex items-center">
          ...
          <label htmlFor="terms" className="ml-2 block text-sm text-gray-900">
            I agree to the{" "}
            <Link href="/terms" className="font-medium text-[#E91E63] hover:text-[#D81B60]">
              Terms of Service
            </Link>{" "}
            and{" "}
            <Link href="/privacy" className="font-medium text-[#E91E63] hover:text-[#D81B60]">
              Privacy Policy
            </Link>
          </label>
        </div>

        <Button type="submit" className="w-full bg-[#E91E63] hover:bg-[#D81B60]" disabled={isLoading}>
          {isLoading ? (
            <>
